...
def init_db():
    ...
    cursor = conn.cursor()
    # បង្កើត Table ជាមួយ Column album_id ឱ្យត្រឹមត្រូវ
...
async def user_cmd(client, message):
    ...
    msg = await message.reply(f"🛰 Checking Erome...", message_thread_id=topic_id)
    ...
    await msg.delete(); await message.reply(f"🏆 Completed {username}!", message_thread_id=topic_id)
...
async def dl_handler(client, message):
    ...
    for i, url in enumerate(urls, 1): 
        await process_album(client, message.chat.id, message.id, url, "general", i, len(urls), topic_id)
...
async def main():
    init_db()
    async with app:
        print("LOG: Tobo Master Fixed & Ready!")
        await idle()

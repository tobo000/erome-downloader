...
async def process_album(client, chat_id, reply_id, url, username, current, total):
    ...
    if pending_p:
        for i in range(0, len(pending_p), 10):
            ...
            chunk = pending_p[i:i+10]
            photo_group = []
            for p_url in chunk:
                p_idx = all_photos.index(p_url)
                p_path = os.path.join(user_folder, f"p_t_{p_idx}.jpg")
                try:
                    r = session.get(p_url, timeout=15)
                    with open(p_path, 'wb') as f:
                        f.write(r.content)
                    
                    real_size = get_human_size(os.path.getsize(p_path))
                    p_info = f"🖼 Photo: `{p_idx + 1}/{len(all_photos)}` | 📦 Size: `{real_size}`"
    ...
    # --- VIDEOS (Using YOUR ORIGINAL working FFmpeg Logic) ---
    for v_idx, v_url in enumerate(all_videos, 1):
        ...
        try:
            r_h = session.head(v_url)
            size = int(r_h.headers.get('content-length', 0))
            ...
            # --- YOUR ORIGINAL FFmpeg Command: Work well for playback ---
...
async def user_cmd(client, message):
    ...
    while True:
        try:
            msg = await message.reply(f"🛰 **Initializing Deep Scanner...**")
            break
        except FloodWait as e:
            await asyncio.sleep(e.value + 1)
...
async def cancel_cmd(client, message):
    cancel_tasks[message.chat.id] = True; backup_to_github()
    await message.reply("🛑 **Cancelled!**")

async def main():
    init_db()
    async with app:
        print("LOG: Full Optimized Bot Started!"); await idle()
